...
function buildRapportDoc(m, logoBuf, bannerBuf, dividerBufs, ctaSchemaBuf) {
  ...
  INSTALLATION_TYPES.forEach(function (t) {
    ...
    if (group && !seenSectionGroups[group.key]) {
      ...
      annexes = annexes.concat(sectionDividerPage(D, group.titre, dividerBufs[group.key], group.images, '5.' + groupNum + ' ' + group.sommaireTitre));
    }
    ...
    children.push(new D.Table({
      ...
      rows: [new D.TableRow({ children: [
        new D.TableCell({
          ...
          // Le bandeau image (banner-rapport.jpg) est un dégradé sans texte — le titre "Rapport
          // d'intervention" du gabarit d'origine est un calque texte séparé par-dessus, pas incrusté
          // dans l'image. Reproduit ici en flottant l'image en arrière-plan du paragraphe (behindDocument)
          // et en superposant le texte blanc dans ce même paragraphe.
          children: [bannerBuf
            ? new D.Paragraph({
                indent: { left: 200 },
                spacing: { before: 100 },
                children: [
                  new D.ImageRun({
                    data: bannerBuf, type: 'jpg', transformation: { width: 520, height: 69 },
                    floating: {
                      horizontalPosition: { relative: D.HorizontalPositionRelativeFrom.COLUMN, offset: 0 },
                  ...
                  new D.TextRun({ text: 'Rapport d’intervention', bold: true, size: 26, color: 'FFFFFF' })
                ]
              })
  ...
  children.push(new D.Table({
    ...
    rows: [new D.TableRow({ children: [
      new D.TableCell({
        ...
        margins: { top: 450, bottom: 450, left: 200, right: 200 },
        children: [
          new D.Paragraph({ spacing: { after: 0 }, children: [new D.TextRun({ text: 'N° d’Affaire : ' + (di.numeroAffaire || '—'), bold: true, size: 20, color: 'FFFFFF' })] }),
          new D.Paragraph({ spacing: { after: 0 }, children: [new D.TextRun({ text: 'Mission réalisée ' + (di.datesIntervention ? ('du ' + di.datesIntervention) : '—'), bold: true, size: 20, color: 'FFFFFF' })] }),
          new D.Paragraph({ spacing: { after: 0 }, children: [new D.TextRun({ text: ' ', size: 12 })] }),
          new D.Paragraph({ children: [new D.TextRun({ text: 'La reproduction de ce document n’est autorisée que sous sa forme intégrale.', italics: true, size: 18, color: 'FFFFFF' })] })
        ]
      })
    ] })]
  }));
...
function threeColBlock(D, labels, values) {
  ...
  function headCell(text) {
    return new D.TableCell({
      ...
      borders: CELL_BORDERS(D),
      verticalAlign: D.VerticalAlign.CENTER,
      margins: { top: 280, bottom: 280, left: 150, right: 150 },
...
function sectionDividerPage(D, titre, imgBufs, imagePaths, sommaireHeading) {
  ...
  var out = [];
  // Entrée de sommaire (5.1 à 5.9) : texte quasi invisible (taille 1pt, blanc) pour ne rien ajouter
  // visuellement à cette page (fidèle au PDF de référence, qui n'affiche que la photo + le bandeau
  // pivoté), tout en restant repéré par Word comme un titre de niveau 2 pour la table des matières —
  // `vanish` a été essayé en premier mais Word l'exclut du scan de mise à jour du sommaire.
  if (sommaireHeading) {
    out.push(new D.Paragraph({
      heading: D.HeadingLevel.HEADING_2,
      spacing: { before: 0, after: 0 },
      children: [new D.TextRun({ text: sommaireHeading, size: 2, color: 'FFFFFF' })]
    }));
  }
